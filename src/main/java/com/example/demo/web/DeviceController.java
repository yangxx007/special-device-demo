...
public class DeviceController {
    ...
    JsonResponse getDeviceLists(@RequestBody DeviceConditions conditions)throws Exception{
        long id=statusService.getCurrUserId(SecurityUtils.getSubject().getSession());
        conditions.setUserId(id);DeviceSearchCondition searchCondition=new DeviceSearchCondition(conditions);
        Pageable pageable = new PageRequest(conditions.getPage(), conditions.getSize(), conditions.getSort());
        CustomePage<DeviceResponse> applyInfos = searchCondition.result(searchCondition.searchByConditions(em),pageable);
